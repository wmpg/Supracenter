...
@cython.cdivision(True)
@cython.nonecheck(False)
cpdef float cyscan(np.ndarray[FLOAT_TYPE_t, ndim=1] supra_pos, np.ndarray[FLOAT_TYPE_t, ndim=1] detec_pos, 
    np.ndarray[FLOAT_TYPE_t, ndim=2] z_profile, wind=True, int n_theta=180, int n_phi=180, float h_tol=330, float v_tol=2000, debug=False):
    ...
    while not found:
        ...
        for i in range(n_layers - 1):
            ...
            az = np.arctan2(cur_x - last_x, cur_y - last_y)
