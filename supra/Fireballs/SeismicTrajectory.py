...
#     try:
#         setup.file_name =       config.get('General', 'file_name')
#         setup.station_file =    config.get('General', 'station_file')
#         setup.mode =            config.get('General', 'mode')
#         setup.enable_winds =    config.get('General', 'enable_winds')
#         #setup.username =        config.get('General', 'username')
#         #setup.password =        config.get('General', 'password')
#         setup.points_name =     config.get('General', 'points_name')

#     except:
#         print('INI ERROR: [General] expected variables: file_name, station_file, mode, enable_winds, points_name')
#         sys.exit()

#     # check if v_fixed is being used
#     try:
#         setup.v_fixed =         float(config.get('General', 'v_fixed'))
#     except:
#         print("INI WARNING: [General] Unable to parse v_fixed, using v instead of v_fixed")
#         setup.v_fixed =         None

#     try:
#         setup.img_dim =         int(config.get('General', 'img_dim'))
#     except:
#         print('INI ERROR: [General] img_dim must be strictly an integer!')
#         sys.exit()


#     try:
#         setup.weather_type =    config.get('Atmospheric', 'weather_type')
#         setup.sounding_file =    config.get('Atmospheric', 'sounding_file')
#         setup.get_data =        config.get('Atmospheric', 'get_data')
#     except:
#         print('INI ERROR: [Atmospheric] expected variables: weather_type, sounding_file, get_data')
#         sys.exit()

#     try:
#         setup.grid_size =       float(config.get('Atmospheric', 'grid_size'))
#     except:
#         print('INI ERROR: [Atmospheric] grid_size must be a float!')
#         sys.exit()
...
#         setup.x0      =         float(config.get('Variables', 'x0'))
#         setup.y0      =         float(config.get('Variables', 'y0'))
#         setup.t0      =         float(config.get('Variables', 't0'))
#         setup.v       =         float(config.get('Variables', 'v'))
#         setup.azim    =         np.radians(float(config.get('Variables', 'azim')))
#         setup.zangle  =         np.radians(90 - float(config.get('Variables', 'zangle')))
#     except:
#         print('INI ERROR: [Variables] v_sound, x0, y0, t0, v, azim, and zangle must be floats!')
#         sys.exit()

#     try:
#         setup.geomode =         config.get('Variables', 'geomode')
#         setup.geomode =         (setup.geomode.lower() == 'true')
#     except:
#         setup.geomode = False
#         print('INI WARNING: [Variables] unable to parse geomode, setting geomode to False')

#     if setup.v_sound < 0:
#         print('INI ERROR: [Variables] v_sound must be greater than 0!')
#         sys.exit()

#     if setup.v < 0:
#         print('INI ERROR: [Variables] v must be greater than 0!')
#         sys.exit()
...
#     except:
#         print('INI ERROR: [Restrictions] azimuth_min, azimuth_max, zangle_min, and zangle_max must be floats!')
#         sys.exit()

#     if setup.azimuth_min >= setup.azimuth_max:
#         print('INI ERROR: [Restrictions] azimuth_min must be smaller than azimuth_max!')
#         sys.exit()

#     if setup.zangle_min >= setup.zangle_max:
#         print('INI ERROR: [Restrictions] zangle_min must be smaller than zangle_max')
#         sys.exit()
...
#     except:
#         print('INI ERROR: [Restrictions] x_min, x_max, y_min, and y_max must be floats!')
#         sys.exit()

#     if setup.x_min >= setup.x_max:
#         print('INI ERROR: [Restrictions] x_min must be smaller than x_max!')
#         sys.exit()

#     if setup.y_min >= setup.y_max:
#         print('INI ERROR: [Restrictions] y_min must be smaller than y_max')
#         sys.exit()
...
#     except:
#         print('INI ERROR: [Restrictions] t_min, t_max, v_min, and v_max must be floats!')
#         sys.exit()

#     if setup.t_min >= setup.t_max:
#         print('INI ERROR: [Restrictions] t_min must be smaller than t_max!')
#         sys.exit()

#     if setup.v_min >= setup.v_max:
#         print('INI ERROR: [Restrictions] v_min must be smaller than v_max')
#         sys.exit()

#     try:
#         setup.maxiter =         int(config.get('Tweaks', 'maxiter'))
#         setup.swarmsize =       int(config.get('Tweaks', 'swarmsize'))
#         setup.run_times =       int(config.get('Tweaks', 'run_times'))
#     except:
#         print('INI ERROR: [Tweaks] maxiter, swarmsize, and run_times must be strictly integers!')
#         sys.exit()

#     if setup.maxiter < 0 or setup.swarmsize < 0 or setup.run_times < 0:
#         print('INI ERROR: [Tweaks] maxiter, swarmsize, and run_times must be greater than zero!')
#         sys.exit()

#     try:
#         setup.phip =            float(config.get('Tweaks', 'phip'))
#         setup.phig =            float(config.get('Tweaks', 'phig'))
#         setup.omega =           float(config.get('Tweaks', 'omega'))
#     except:
#         print('INI ERROR: [Tweaks] phip, phig and omega must be floats!')
#         sys.exit()

#     if setup.phip < 0 or setup.phig < 0 or setup.omega < 0:
#         print('INI ERROR: [Tweaks] phip, phig and omega must be greater than zero!')
#         sys.exit()

#     try:
#         setup.debug =           config.get('Tweaks', 'debug')
#     except:
#         print('INI ERROR: [Tweaks] expected debug variable!')
#         sys.exit()

#     # initialize other variables which are based off of the variables from the ini
