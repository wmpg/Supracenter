...
class SolutionGUI(QMainWindow):
    ...
    def supSearch(self, manual):
        supSearch(self.bam, self.prefs, manual=manual)
    ...
    def residPlot(self, results_arr, s_name, xstn, output_name, n_stations, manual=True):
        ...
        for h in range(n_stations):
            ...
            # Add station names
            ax.text(xstn[h, 1], xstn[h, 0],  '%s' % (s_name[h]), size=10, zorder=1, color='w')
        ...
        for ptb in range(self.setup.perturb_times):
            ...
            ax.scatter(results_arr[ptb].x_opt.lon, results_arr[ptb].x_opt.lat, c="g", marker='*', s=21, alpha=0.7)
